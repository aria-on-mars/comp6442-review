...
public class NonEmptyBinaryTree <T extends Comparable<T>> extends BinaryTree<T> {
	...
	public BinaryTree<T> insert(T d) {
		...
		if (data.compareTo(d) == -1) {
			if (right.isEmpty()) {
				right = new NonEmptyBinaryTree<T>(d);
			} else {
				right.insert(d);
			}
		} else if (data.compareTo(d) == 1){
			if (left.isEmpty()) {
				left = new NonEmptyBinaryTree<T>(d);
			} else {
				left.insert(d);
			}
		}
	...
	public BinaryTree<T> delete(T d) {
		...
		if (data.compareTo(d) == 0) {
			if (left.isEmpty()) {
				...
				data = (T) ((NonEmptyBinaryTree)right).findSuccessor(this);
			}
		} else if (data.compareTo(d) == -1) {
	...
	public T findSuccessor(BinaryTree<T> t) {
		if (left.isEmpty()) {
			t.delete(data);
			return data;
		} else {
			return (T) ((NonEmptyBinaryTree)left).findSuccessor(this);
		}
	}
